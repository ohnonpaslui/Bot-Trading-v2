...
RSI_PERIODE = 14
RSI_SURVENTE = 35         # signal d'achat sous ce seuil (assoupli, était 30)
RSI_SURACHAT = 65         # signal de vente au-dessus de ce seuil (assoupli, était 70)
...
def signal_tendance(closes, court=5, long=20):
    """Détecte un croisement de moyennes mobiles (court terme au-dessus/en
    dessous du long terme) : complète le RSI pour capter les mouvements de
    tendance (marché qui monte/descend sans repasser par les zones de
    survente/surachat). Renvoie (achat, vente) : True/False chacun, selon
    qu'un croisement vient de se produire à l'instant présent."""
    ...
    if None in (sma_court_prec, sma_long_prec, sma_court, sma_long):
        return False, False
    achat = sma_court_prec <= sma_long_prec and sma_court > sma_long   # croisement haussier
    vente = sma_court_prec >= sma_long_prec and sma_court < sma_long   # croisement baissier
    return achat, vente
...
def run_daily():
    ...
    if sortie_forcee:
        ...
        # Signal 1 (retournement) : le RSI ÉTAIT en survente/surachat et vient d'en ressortir
        signal_achat_rsi = rsi_prec < RSI_SURVENTE and rsi >= RSI_SURVENTE
        signal_vente_rsi = rsi_prec > RSI_SURACHAT and rsi <= RSI_SURACHAT

        # Signal 2 (tendance) : croisement de moyennes mobiles, capte les mouvements
        # qui ne repassent jamais par les zones de survente/surachat du RSI
        signal_achat_tendance, signal_vente_tendance = signal_tendance(closes)

        signal_achat = signal_achat_rsi or signal_achat_tendance
        signal_vente = signal_vente_rsi or signal_vente_tendance
        origine_achat = "retournement RSI" if signal_achat_rsi else ("tendance haussière" if signal_achat_tendance else "")
