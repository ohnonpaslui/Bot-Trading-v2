...
MARGE_CASSURE = 0.002  # 0.2% : dépassement minimal exigé pour valider une cassure (filtre les faux breakouts)
...
def detect_pattern(candles):
    """Détecte une figure de bougie japonaise parmi 21 figures classiques
    (reconnues et documentées, pas inventées), en analysant les 2-3
    dernières bougies. Sert uniquement à alimenter `motif_chandelier` pour
    la transparence du journal — le blocage réel des achats se fait via
    `biais_directionnel`, qui combine plusieurs bougies + RSI + volume."""
    if len(candles) < 2:
        return None
    ...
    def milieu(c): return (c["open"] + c["close"]) / 2

    b, r = body(last), range_(last)
    tol = 0.0015  # 0.15% : tolérance pour considérer deux niveaux "égaux" (tweezers, counter attack)

    # ---------- Figures sur 3 bougies (vérifiées en premier, les plus spécifiques) ----------
    if len(candles) >= 3:
        c1, c2, c3 = candles[-3], candles[-2], candles[-1]
        b1 = body(c1)

        if is_bearish(c1) and body(c2) < 0.3 * b1 and is_bullish(c3) and c3["close"] > milieu(c1):
            return "Étoile du matin (Morning Star, retournement haussier)"
        if is_bullish(c1) and body(c2) < 0.3 * b1 and is_bearish(c3) and c3["close"] < milieu(c1):
            return "Étoile du soir (Evening Star, retournement baissier)"
        if (is_bullish(c1) and is_bullish(c2) and is_bullish(c3)
            ...
            return "Trois soldats blancs (continuation haussière)"
        if (is_bearish(c1) and is_bearish(c2) and is_bearish(c3)
                and c2["close"] < c1["close"] and c3["close"] < c2["close"]
                and c2["open"] < c1["open"] and c3["open"] < c2["open"]):
            return "Trois corbeaux noirs (continuation baissière)"
        # Three Inside Up/Down : Harami + bougie de confirmation
        if (is_bearish(c1) and is_bullish(c2) and c2["open"] > c1["close"] and c2["close"] < c1["open"]
                and is_bullish(c3) and c3["close"] > c1["open"]):
            return "Three Inside Up (retournement haussier confirmé)"
        if (is_bullish(c1) and is_bearish(c2) and c2["open"] < c1["close"] and c2["close"] > c1["open"]
                and is_bearish(c3) and c3["close"] < c1["open"]):
            return "Three Inside Down (retournement baissier confirmé)"

    # ---------- Figures sur 2 bougies ----------
    ...
    # Même forme que Marteau/Étoile filante, mais contexte de tendance inversé
    if upper_wick(last) >= 2 * b and lower_wick(last) <= 0.3 * b and prev["close"] > last["close"]:
        return "Marteau inversé (retournement haussier potentiel)"
    if lower_wick(last) >= 2 * b and upper_wick(last) <= 0.3 * b and prev["close"] < last["close"]:
        return "Pendu / Hanging Man (retournement baissier potentiel)"
    ...
    if (is_bearish(prev) and is_bullish(last) and last["open"] < prev["close"]
            and milieu(prev) < last["close"] < prev["open"]):
        return "Piercing Pattern (retournement haussier)"
    if (is_bullish(prev) and is_bearish(last) and last["open"] > prev["close"]
            and prev["open"] < last["close"] < milieu(prev)):
        return "Dark Cloud Cover (retournement baissier)"

    if (is_bearish(prev) and is_bullish(last) and last["open"] > prev["close"]
            and last["close"] < prev["open"] and body(last) < body(prev) * 0.6):
        return "Bullish Harami (retournement haussier)"
    if (is_bullish(prev) and is_bearish(last) and last["open"] < prev["close"]
            and last["close"] > prev["open"] and body(last) < body(prev) * 0.6):
        return "Bearish Harami (retournement baissier)"

    if is_bearish(prev) and is_bullish(last) and abs(prev["low"] - last["low"]) <= tol * prev["low"]:
        return "Tweezer Bottom (retournement haussier)"
    if is_bullish(prev) and is_bearish(last) and abs(prev["high"] - last["high"]) <= tol * prev["high"]:
        return "Tweezer Top (retournement baissier)"

    if (is_bearish(prev) and is_bullish(last) and last["open"] < prev["close"]
            and abs(prev["close"] - last["close"]) <= tol * prev["close"]):
        return "Bullish Counter Attack (retournement haussier)"
    if (is_bullish(prev) and is_bearish(last) and last["open"] > prev["close"]
            and abs(prev["close"] - last["close"]) <= tol * prev["close"]):
        return "Bearish Counter Attack (retournement baissier)"
...
def biais_directionnel(candles, rsi, rsi_prec, vol_ok, n=5):
    """Détermine un biais directionnel (haussier/baissier/neutre) en combinant
    plusieurs facteurs, plutôt que de se limiter à bloquer un achat sur une
    seule bougie baissière isolée :
      - la composition des `n` dernières bougies (majorité haussière ou baissière)
      - la direction du RSI (monte ou descend par rapport à l'exécution précédente)
      - le volume (sert de confiance : sans confirmation de volume, le biais
        est atténué plutôt qu'ignoré)
    Renvoie une chaîne : "haussier", "baissier" ou "neutre"."""
    if len(candles) < n:
        return "neutre"
    recentes = candles[-n:]
    haussieres = sum(1 for c in recentes if c["close"] > c["open"])
    baissieres = sum(1 for c in recentes if c["close"] < c["open"])
    score = haussieres - baissieres  # entre -n et +n

    if rsi is not None and rsi_prec is not None:
        if rsi > rsi_prec:
            score += 1
        elif rsi < rsi_prec:
            score -= 1
    ...
    if not vol_ok:
        score = score / 2  # sans confirmation de volume, on atténue la conviction du biais
...
def traiter_actif(actif, fg_value, fg_label):
    ...
    biais = biais_directionnel(candles, rsi, rsi_prec, vol_ok)
    ...
    # Risque initial en unité de prix = 1R. Sert à normaliser la performance
    # de chaque trade (pnl_r = combien de fois le risque initial a été gagné/perdu),
    # indépendamment du % de réussite brut.
    risque_initial = (ATR_MULT_SL * atr_entree) if atr_entree else (STOP_LOSS_PCT_DEFAUT * prix_entree if prix_entree else 0)
    ...
    if sortie_forcee:
        ...
        pnl_r = round((price - prix_entree) / risque_initial, 2) if risque_initial else ""
        ...
        signal_achat_breakout = (
            ...
            and price > resistance_proche * (1 + MARGE_CASSURE)
        )
        ...
        # Le biais directionnel (bougies récentes + direction du RSI + volume)
        # remplace l'ancien veto sur une seule bougie baissière isolée : on
        # exige maintenant que l'ensemble de ces facteurs ne pointe pas
        # clairement à la baisse avant d'valider un achat.
        if signal_achat and biais == "baissier":
            signal_achat = False
            blocage = f"biais directionnel baissier (bougies + RSI + volume)"
...
def rapport():
    for actif in ACTIFS:
        ...
        if trades:
            ...
            # --- Stats en unité R (comme un trader : le taux de réussite seul ne
            # dit rien, ce qui compte c'est le R total et le ratio gains/pertes) ---
            trades_avec_r = [t for t in trades if t.get("pnl_r") not in (None, "")]
            if trades_avec_r:
                valeurs_r = [float(t["pnl_r"]) for t in trades_avec_r]
                gains_r = [v for v in valeurs_r if v > 0]
                pertes_r = [v for v in valeurs_r if v <= 0]
                r_total = sum(valeurs_r)
                gain_moyen_r = sum(gains_r) / len(gains_r) if gains_r else 0
                perte_moyenne_r = sum(pertes_r) / len(pertes_r) if pertes_r else 0
                print(f"R total cumulé : {r_total:+.2f}R")
                print(f"Gain moyen : {gain_moyen_r:+.2f}R | Perte moyenne : {perte_moyenne_r:+.2f}R")
                if perte_moyenne_r != 0:
                    print(f"Ratio risque/récompense réalisé : 1 : {abs(gain_moyen_r/perte_moyenne_r):.2f}")
