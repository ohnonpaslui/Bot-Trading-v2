...
def get_intraday(symbole, timeframe="30Min", limite=100):
    ...
    url = (
        ...
        f"?timeframe={timeframe}&start={depuis}&limit={limite}&feed=iex&sort=desc"
    )
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as r:
            ...
            candles.reverse()  # l'API renvoie du plus récent au plus ancien avec sort=desc ;
                                # on remet dans l'ordre chronologique (plus ancien -> plus récent)
                                # attendu par le reste du code (SMA, RSI, closes[-1] = dernier prix)
            return candles
